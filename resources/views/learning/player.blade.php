<x-app-layout>
            ...
            <div id="tab-content" class="{{ $enrollment->learning_type === 'sync' ? '' : '' }}">
                ...
                <div class="max-w-4xl mx-auto py-8 px-6">
                    ...
                    <div class="flex justify-center mb-8">
                        @if(!$canMarkComplete && !in_array($currentLesson->id, $completedLessonIds))
                        {{-- Timer Active: Show countdown + disabled button --}}
                        <div class="text-center">
                            <div class="mb-2">
                                <span class="text-sm text-orange-600 font-medium">
                                    ⏱ Mark Complete available in <span id="timer-display">{{ sprintf('%d:%02d', intdiv($remainingSeconds ?? 0, 60), ($remainingSeconds ?? 0) % 60) }}</span>
                                </span>
                            </div>
                            <button disabled
                                class="px-8 py-3 rounded-xl font-semibold text-lg bg-gray-200 text-gray-500 cursor-not-allowed">
                                Mark as Complete
                            </button>
                            <p class="text-xs text-gray-400 mt-2">Please spend at least {{ $course->lesson_min_minutes ?? 1 }} minutes on this lesson.</p>
                        </div>
                        @elseif(in_array($currentLesson->id, $completedLessonIds))
                        {{-- Already Completed --}}
                        <button id="mark-complete-btn"
                            data-course-slug="{{ $course->slug }}"
                            data-lesson-id="{{ $currentLesson->id }}"
                            class="px-8 py-3 rounded-xl font-semibold text-lg transition-all duration-200 bg-success text-white hover:bg-success-dark">
                        ...
                        <button id="mark-complete-btn"
                            data-course-slug="{{ $course->slug }}"
                            data-lesson-id="{{ $currentLesson->id }}"
                            class="px-8 py-3 rounded-xl font-semibold text-lg transition-all duration-200 bg-white border-2 border-accent text-accent hover:bg-accent hover:text-white">
                            Mark as Complete
                        </button>
                        @endif
                    </div>
   ...
   <script>
    @if(isset($currentLesson) && !$canMarkComplete && !in_array($currentLesson->id, $completedLessonIds) && ($remainingSeconds ?? 0) > 0)
        (function() {
            let remainingSeconds = {{ (int)($remainingSeconds ?? 0) }};
            const timerDisplay = document.getElementById('timer-display');
            const timerContainer = timerDisplay ? timerDisplay.closest('.text-center') : null;

            if (!timerDisplay || remainingSeconds <= 0) return;
    ...
    function attachMarkCompleteEvent() {
        ...
        markBtn.addEventListener('click', function() {
            ...
            .then(async response => {
                ...
                if (!response.ok) {
                    const message = data?.error || 'Unable to mark lesson complete.';
                    alert(message);
                    this.disabled = false;
