...
class LearningTest extends TestCase
{
    ...
    public function test_lesson_page_refresh_preserves_timer_state(): void
    {
        [$course, $lesson] = $this->courseWithLesson();
        $user = User::factory()->create();
        Enrollment::factory()->create([
        ...
        $startTime = now()->subSeconds(30)->toIso8601String();

        $this->actingAs($user)
            ->withSession([
                "lesson_start_{$lesson->id}" => $startTime,
            ])
            ->get(route('learning.lesson', [$course->slug, $lesson->id]))
            ->assertOk()
            ->assertViewHas('canMarkComplete', false)
            ->assertViewHas('remainingSeconds', fn ($seconds) => $seconds > 0 && $seconds <= 60);
    }
